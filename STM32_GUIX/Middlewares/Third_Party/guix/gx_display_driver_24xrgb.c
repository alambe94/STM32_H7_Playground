...
__attribute__((section(".SD_RAM"))) static ULONG frame_buffer[STM32F746G_DISCO_SCREEN_WIDTH][STM32F746G_DISCO_SCREEN_HEIGHT];
...
INT          width;
    ...
    if (_gx_utility_rectangle_overlap_detect(&dirty, &composite -> gx_canvas_dirty_area, &overlap))
    {
        ...
        gx_dma_init(DMA2D_M2M, (uint32_t) write, STM32F746G_DISCO_SCREEN_WIDTH - width, width, overlap.gx_rectangle_bottom - overlap.gx_rectangle_top + 1);  
...
/**************************************************************************/
static void stm32_24xrgb_buffer_toggle(GX_CANVAS *canvas, GX_RECTANGLE *dirty)
{
GX_RECTANGLE    Limit;
GX_RECTANGLE    Copy;
ULONG           offset;
...
ULONG *put;
    ...
    if (gx_utility_rectangle_overlap_detect(&Limit, &canvas->gx_canvas_dirty_area, &Copy))
    {
        ...
        offset = (canvas->gx_canvas_display_offset_y + Copy.gx_rectangle_top)* STM32F746G_DISCO_SCREEN_WIDTH;
        offset += canvas->gx_canvas_display_offset_x + Copy.gx_rectangle_left;
        put += offset;
        

        #if defined(GX_CHROMEART_ENABLE)
        gx_dma_init(DMA2D_M2M_PFC, (uint32_t) put, STM32F746G_DISCO_SCREEN_WIDTH - copy_width, copy_width, copy_height);  
        ...
        dest_stride_ulongs = STM32F746G_DISCO_SCREEN_WIDTH;
...
/*                                                                        */
/**************************************************************************/
UINT stm32_graphics_driver_setup_24xrgb(GX_DISPLAY *display)
{
    ...
    */
    _gx_display_driver_24xrgb_setup(display, (VOID*)LCD_SCREEN_HANDLE, stm32_24xrgb_buffer_toggle);
