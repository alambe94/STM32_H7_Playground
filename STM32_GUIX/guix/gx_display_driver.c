...
__attribute__((section(".SD_RAM"))) static USHORT frame_buffer[LCD_DEFAULT_WIDTH][LCD_DEFAULT_HEIGHT];
...
INT          width;
    ...
    if (_gx_utility_rectangle_overlap_detect(&dirty, &composite -> gx_canvas_dirty_area, &overlap))
    {
        ...
        gx_dma_init(DMA2D_M2M, (uint32_t) write, LCD_DEFAULT_WIDTH - width, width, overlap.gx_rectangle_bottom - overlap.gx_rectangle_top + 1);
...
/**************************************************************************/
static void stm32f746_565rgb_buffer_toggle(GX_CANVAS *canvas, GX_RECTANGLE *dirty)
{
GX_RECTANGLE    Limit;
GX_RECTANGLE    Copy;
ULONG           offset;
...
ULONG *put;
    ...
    if (gx_utility_rectangle_overlap_detect(&Limit, &canvas->gx_canvas_dirty_area, &Copy))
    {
        ...
        offset = (canvas->gx_canvas_display_offset_y + Copy.gx_rectangle_top)* LCD_DEFAULT_WIDTH;
        ...
        #if defined(GX_CHROMEART_ENABLE)
        gx_dma_init(DMA2D_M2M, (uint32_t) put, LCD_DEFAULT_WIDTH - copy_width, copy_width, copy_height);
        ...
        dest_stride_ulongs = LCD_DEFAULT_WIDTH / 2;
...
/*                                                                        */
/**************************************************************************/
UINT stm32_graphics_driver_setup_565rgb(GX_DISPLAY *display)
{
    ...
    */
    _gx_display_driver_565rgb_setup(display, (VOID*)LCD_SCREEN_HANDLE, stm32f746_565rgb_buffer_toggle);
    ...
    BSP_LCD_SetLayerAddress(0, 0, (uint32_t)&frame_buffer);
    ...
    memset(frame_buffer, 0, LCD_DEFAULT_WIDTH * LCD_DEFAULT_HEIGHT * 2);
