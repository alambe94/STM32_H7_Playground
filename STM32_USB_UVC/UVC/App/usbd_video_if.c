...
/* USER CODE BEGIN PRIVATE_VARIABLES */
   uint16_t YUY_Image[UVC_MAX_FRAME_SIZE];
...
static int8_t VIDEO_Itf_Init(void)
{
  ...
  */
  for(uint32_t i=0; i<sizeof(YUY_Image); i++)
  {
	  YUY_Image[i]= i;
  }
...
static int8_t VIDEO_Itf_Data(uint8_t **pbuf, uint16_t *psize, uint16_t *new_frame)
{
  ...
  uint16_t PcktSze = MIN(sizeof(YUY_Image) - picture_index, UVC_PACKET_SIZE - 2);

  *pbuf = (uint8_t *)(YUY_Image+picture_index);
