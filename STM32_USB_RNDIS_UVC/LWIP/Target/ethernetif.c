...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  (void)USBD_CDC_RNDIS_SetTxBuffer(&hUsbDeviceHS, hcdc->TxBuffer, hcdc->TxLength);

  /* Start transmitting the Tx buffer */
  if (USBD_CDC_RNDIS_TransmitPacket(&hUsbDeviceHS) == (uint8_t)USBD_OK)
  {
    return (err_t)ERR_OK;
  }
  UNUSED(netif);
...
void ethernetif_input(struct netif *pnetif)
{
  ...
  (void) USBD_LL_PrepareReceive(&hUsbDeviceHS,
                         CDC_RNDIS_OUT_EP,
...
void ethernetif_update_config(struct netif *netif)
{
  ...
  {
	  // TODO BSP_LED_On(LED1);
  }
  else
  {
	  // TODO BSP_LED_Off(LED1);
  }
