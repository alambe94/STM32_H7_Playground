...
static uint8_t USBD_CDC_RNDIS_Init(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  if (hcdc == NULL)
  {
    pdev->pClassDataRNDIS = NULL;
    return (uint8_t)USBD_EMEM;
  }

  pdev->pClassDataRNDIS = (void *)hcdc;
  ...
  ((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->Init();
...
static uint8_t USBD_CDC_RNDIS_DeInit(USBD_HandleTypeDef *pdev, uint8_t cfgidx)
{
  ...
  if (pdev->pClassDataRNDIS != NULL)
  {
    ((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->DeInit();
    USBD_free(pdev->pClassDataRNDIS);
    pdev->pClassDataRNDIS = NULL;
  }
...
static uint8_t USBD_CDC_RNDIS_Setup(USBD_HandleTypeDef *pdev,
                                    USBD_SetupReqTypedef *req)
{
  USBD_CDC_RNDIS_HandleTypeDef *hcdc = (USBD_CDC_RNDIS_HandleTypeDef *)pdev->pClassDataRNDIS;
    ...
    case USB_REQ_TYPE_CLASS :
      if (req->wLength != 0U)
      {
        ...
        if ((req->bmRequest & 0x80U) != 0U)
        {
          ...
          ((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->Control(req->bRequest,
                                                                  (uint8_t *)hcdc->data,
                                                                  req->wLength);
...
static uint8_t USBD_CDC_RNDIS_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  hcdc = (USBD_CDC_RNDIS_HandleTypeDef *)pdev->pClassDataRNDIS;
  ...
  if (epnum == (CDC_RNDIS_IN_EP & 0x7FU))
  {
    ...
    else
    {
      ...
      if (((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->TransmitCplt != NULL)
      {
        ((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->TransmitCplt(hcdc->TxBuffer, &hcdc->TxLength, epnum);
      }
    }
  }
...
static uint8_t USBD_CDC_RNDIS_EP0_RxReady(USBD_HandleTypeDef *pdev)
{
  ...
  if ((pdev->pUserDataRNDIS != NULL) && (hcdc->CmdOpCode != 0xFFU))
  {
...
static uint8_t *USBD_CDC_RNDIS_USRStringDescriptor(USBD_HandleTypeDef *pdev, uint8_t index, uint16_t *length)
{
  ...
  {
    USBD_GetString((uint8_t *)((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->pStrDesc, USBD_StrDesc, length);
    return USBD_StrDesc;
  }
                                               ...
                                               USBD_CDC_RNDIS_PacketMsgTypeDef *Msg)
{
  ...
  ((USBD_CDC_RNDIS_ItfTypeDef *)pdev->pUserDataRNDIS)->Receive(hcdc->RxBuffer, &hcdc->RxLength);
