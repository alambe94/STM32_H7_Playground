...
static USBD_VideoControlTypeDef video_Commit_Control =
{
  ...
  .bFormatIndex = 0x01U,
  .bFrameIndex = 0x01U,
  .dwFrameInterval = UVC_INTERVAL(UVC_CAM_FPS_HS),
...
static uint8_t  USBD_VIDEO_DataIn(USBD_HandleTypeDef *pdev, uint8_t epnum)
{
  ...
  if (hVIDEO->uvc_state == UVC_PLAY_STATUS_STREAMING)
  {
    ...
    ((USBD_VIDEO_ItfTypeDef *)pdev->pUserData)->Data(&Pcktdata, &PcktSze, &NewFrame);
    ...
    packet[0] = 0x02;
	  ...
	  (void)USBD_memcpy((packet + 2U), Pcktdata, PcktSze);
